...
function initSliders() {
  ...
  if (document.querySelector(".selector-row__slider")) {
    new Swiper(".selector-row__slider", {
      // <- Вказуємо склас потрібного слайдера
      ...
      // freeMode: true,
      navigation: {
        prevEl: ".selector-row__prev",
        nextEl: ".selector-row__next"
      },
      scrollbar: {
        el: ".selector-row__scrollbar.swiper-scrollbar",
        draggable: true,
        hide: false
        // показує scrollbar постійно
      }
    });
  }
  if (document.querySelector(".selector-row__slider-1")) {
    new Swiper(".selector-row__slider-1", {
      // <- Вказуємо склас потрібного слайдера
      modules: [Navigation, Scrollbar],
      observer: true,
      observeParents: true,
      ...
      speed: 800,
      navigation: {
        prevEl: ".selector-row__prev-1",
        nextEl: ".selector-row__next-1"
      },
      scrollbar: {
        el: ".selector-row__scrollbar-1.swiper-scrollbar",
        draggable: true,
        hide: false
        // показує scrollbar постійно
      }
  ...
  if (document.querySelector(".selector-row__slider-2")) {
    new Swiper(".selector-row__slider-2", {
      // <- Вказуємо склас потрібного слайдера
      modules: [Navigation, Scrollbar],
      observer: true,
      observeParents: true,
      ...
      speed: 800,
      navigation: {
        prevEl: ".selector-row__prev-2",
        nextEl: ".selector-row__next-2"
      },
      scrollbar: {
        el: ".selector-row__scrollbar-2.swiper-scrollbar",
        draggable: true,
        hide: false
        // показує scrollbar постійно
      }
...
function setupInteractiveButton({
  ...
  imageSpeedAttribute,
  likeMode = false
  // toggle режим для лайків
}) {
  document.addEventListener("click", function(e) {
    const targetElement = e.target;
    const button = targetElement.closest(buttonSelector);
    if (!button) return;
    const counterElement = document.querySelector(counterSelector);
    const product = button.closest(productSelector);
    const imageElement = product ? product.querySelector(imageSelector) : null;
    let quantity = 1;
    if (!likeMode && product) {
      const quantityElement = product.querySelector(quantitySelector);
      if (quantityElement) {
    ...
    if (likeMode) {
      ...
      if (counterElement) {
        counterElement.innerHTML = +counterElement.innerHTML + (liked ? -1 : 1);
      }
      if (!liked && imageElement && counterElement) {
        animateFlyToCounter(imageElement, counterElement, imageSpeedAttribute);
      }
      return;
    }
    ...
    if (counterElement) {
      counterElement.innerHTML = +counterElement.innerHTML + quantity;
    }
  });
}
function animateFlyToCounter(imageElement, counterElement, speedAttr) {
  const speed = +imageElement.dataset[speedAttr] || 500;
  ...
  setTimeout(() => {
    ...
    flyImg.style.left = `${counterRect.left}px`;
    flyImg.style.top = `${counterRect.top}px`;
...
if (document.querySelector("[data-fls-addtocart]")) {
  window.addEventListener("load", () => {
    setupInteractiveButton({
      buttonSelector: "[data-fls-addtocart-button]",
      counterSelector: "[data-fls-addtocart]",
      productSelector: "[data-fls-addtocart-product]",
      quantitySelector: "[data-fls-addtocart-quantity]",
      imageSelector: "[data-fls-addtocart-image]",
      imageSpeedAttribute: "flsAddtocartImage"
    });
  });
}
if (document.querySelector("[data-fls-like]")) {
  window.addEventListener("load", () => {
    setupInteractiveButton({
      buttonSelector: "[data-fls-like-button]",
      counterSelector: "[data-fls-like]",
      productSelector: "[data-fls-like-product]",
      quantitySelector: "",
      // не потрібно
      imageSelector: "[data-fls-like-image]",
      imageSpeedAttribute: "flsLikeImage",
      likeMode: true
      // toggle режим
    });
  });
